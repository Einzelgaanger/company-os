...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "field-input flex h-10 w-full items-center justify-between text-left [&>span]:line-clamp-1",
      className
    )}
    {...props}
  >
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-md border border-border bg-surface text-ink shadow-md data-[state=open]:animate-in data-[state=open]:fade-in-0",
